const setSearchIdStub = require('./stubs/set-search-id-stub');
const setTicketsStub = require('./stubs/set-tickets-stub');
...
describe('Страница поиска билетов', () => {
  beforeEach(async () => {
    ...
    page.on('request', (interceptedRequest) => {
      if (interceptedRequest.url().includes('/search')) {
        return setSearchIdStub(interceptedRequest);
      }
      if (interceptedRequest.url().includes('/tickets')) {
        return setTicketsStub(interceptedRequest);
      }
  ...
  describe('Сортировка', () => {
    ...
    it('По умолчанию в адресной строке отображается вариант «Самый дешевый»', async () => {
      const urlSearchParams = await page.evaluate(() => location.search);
